...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        // Set the Watson credentials for Tone Analyzer service from the WatsonCredentials.plist
        let toneAnalyzerPassword = configuration?["ToneAnalyzerPassword"] as! String
        let toneAnalyzerUsername = configuration?["ToneAnalyzerUsername"] as! String
        // Set date string for version of Watson service to use
        let versionDate = "2016-10-15"
        // Initialize Tone Analyzer object
        toneAnalyzer = ToneAnalyzer(username: toneAnalyzerUsername, password: toneAnalyzerPassword, version: versionDate)
    ...
    func analyzeTone(_ textToAnalyze: String){
        ...
        toneAnalyzer.getTone(ofText:textToAnalyze, failure: failToneAnalyzerWithError) { tones in
            // Loop through sentence tones
            self.analyzedCategoriesArray = []
            // Loop through document tones
            for documentTone in tones.documentTone{
                // Set tone category parameters
                let toneCategoryId = documentTone.categoryID
                let toneCategoryName = documentTone.name
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAtIndexPath indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if(analyzedCategoriesArray.indices.contains(collectionView.tag)){
            ...
            cell.tagLabel.text = self.analyzedCategoriesArray[collectionView.tag].tones[indexPath.item].name
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAtIndexPath indexPath: IndexPath) -> CGSize {
            ...
            let size = NSString(string: self.analyzedCategoriesArray[collectionView.tag].tones[indexPath.item].name).size(attributes: nil)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: IndexPath){
        ...
        if(cell?.tagLabel.text == self.analyzedCategoriesArray[collectionView.tag].tones[indexPath.item].name){
